...
configure_firewall() {
  ...
  case "$FLUXOMNI_INSTALL_TARGET" in
    media-node)
      ...
      ;;
    *)
      tcp_ports=(80 443 1935 8000)
      udp_ports=(8000 10080)
      if [ -n "$REQUESTED_CONTROL_PLANE_RPC_PORT" ] &&
        ! is_local_port_mapping "$REQUESTED_CONTROL_PLANE_RPC_PORT"; then
        tcp_ports+=("$(published_port "$REQUESTED_CONTROL_PLANE_RPC_PORT")")
      fi
      ;;
  esac
