...
write_summary() {
...

  {
    ...
    printf -- "- Fluxomni Studio version: \`%s\`\n" "$FLUXOMNI_VERSION"
...
verify_provisioning() {
  ...
  echo "Verifying Fluxomni Studio installation on the server"
  ...
  if [ "$http_ok" -ne 1 ]; then
    echo "Error: Fluxomni Studio UI did not become reachable at http://${SERVER_IP}/" >&2
    exit 1
  fi
}
