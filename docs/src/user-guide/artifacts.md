...
Artifacts are reusable source files and generated media assets that Fluxomni Studio tracks in a shared library. Use this page to import files once, tag them, monitor storage capacity, and reuse them from route playlists or fallback sources.
...
Fluxomni Studio rejects uploads before they exceed the backend admission limit. The rejected upload also appears in Attention as a storage alert so operators can free space or move content before retrying.
