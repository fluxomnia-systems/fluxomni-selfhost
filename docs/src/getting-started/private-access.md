...
Run Fluxomni Studio on a private overlay network when the Control Surface and media ports should be reachable only from your own devices or servers.

This page shows Tailscale first because it is the easiest private-network option for most self-host deployments. The same Fluxomni Studio rule applies to every option: set the advertised hosts to the address that your publishers, viewers, and extra media nodes can actually reach.
...
Add the server as a new device in Tailscale, join your tailnet, then install Fluxomni Studio with the server's Tailscale IP as the advertised host.
...
# Install Fluxomni Studio so generated UI, RTMP, HLS, and SRT URLs use the tailnet IP.
curl -fsSL https://install.fluxomni.io | \
  FLUXOMNI_PUBLIC_HOST="$TS_IP" \
  FLUXOMNI_MEDIA_NODE_PUBLIC_HOST="$TS_IP" \
  bash

printf '\nFluxomni Studio Control Surface: http://%s\n' "$TS_IP"
...
printf '\nFluxomni Studio Control Surface: https://%s\n' "$TS_DNS"
```
...
Use Tailscale ACLs to restrict who can reach the Fluxomni Studio host. This example allows only members of `group:operators` to reach the UI and media ports.
...
For distributed Fluxomni Studio over Tailscale, keep these TCP paths reachable:
