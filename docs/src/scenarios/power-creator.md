...
Single-host Fluxomni Studio on a small cloud VM (Hetzner CPX21, €5–10/mo). OBS on your desk pushes RTMP to Fluxomni Studio, which fans out to YouTube, Twitch, Kick, Facebook, and any custom RTMP endpoint.

```text
OBS → Fluxomni Studio (single VM) → { YouTube, Twitch, Kick, Facebook, Custom RTMP }
```
